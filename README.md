...
**通过自然语言控制 Android 手机的 GUI Agent**
...
Phone Agent 是一个基于视觉语言模型（VLM）的 Android GUI 自动化工具，能够理解屏幕内容并将自然语言指令转换为手机操作序列。
...
python -m phone_agent run "打开抖音我的喜欢里搜索跳舞的视频"
```
...
# 可选配置
PHONE_AGENT_DEVICE_SERIAL=       # 多设备时指定
PHONE_AGENT_MAX_STEPS=45         # 单任务最大步数
PHONE_AGENT_THINKING_MODE=enabled # enabled|disabled
```
...
> 视觉兜底成功后自动学习映射，持久化到 `~/.phone_agent/learned_apps.json`
...
> 默认 `enabled`（准确率优先），可通过 `PHONE_AGENT_THINKING_MODE` 环境变量调整
...
- **Issues**: [GitHub Issues](https://github.com/your-username/phone-agent/issues)
- **Discussions**: [GitHub Discussions](https://github.com/your-username/phone-agent/discussions)
